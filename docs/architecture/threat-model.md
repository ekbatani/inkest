# Inkest Versioned Security Threat Model (SEC-THREATS)
...
Each named threat is verified through automated and manual test flows:
- **T-01 & T-08:** Tested by cross-user security audit script (`P0-40`) and database scoping tests.
- **T-02:** Tested by sanitized Markdown preview test vectors (`src/components/markdown/`).
- **T-03:** Verified via Argon2id hash inspection and Passkey enrollment tests (`R6-01`, `R6-02`).
- **T-04:** Verified by NextAuth callback origin validation and CSRF token tests.
- **T-05:** Verified by `src/server/attachments/validation.test.ts`.
- **T-06:** Verified by staged prompt-injection tests (`src/server/ai/provider.test.ts`).
- **T-07:** Verified by client crypto vault lock tests (`src/lib/vault-crypto.ts`).

---

## 4. Security Maintenance & Release Review Schedule

1. **Pre-Release Audit:** This document MUST be reviewed and updated prior to any minor/major release tag.
2. **Automated Verification:** Continuous security check scripts (`bun run test`) test authorization scoping and payload validation against T-01 through T-08.
